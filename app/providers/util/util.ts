...
export class Util {
  ...
  loadRuliweb(_url, _page) {
    let url = _url + _page;
    this.http.get(url).subscribe(data => {
      let doc = this.getDocFromData(data);
      let elements = doc.querySelectorAll('table.board_list_table tr.table_body:not(.notice)');
      debugger;
  ...
  loadRuliweb2(_url, _page) {
        ...
        return new Observable(observer => {
          ...
          this.http.get(url).subscribe(data => {
            let doc = this.getDocFromData(data);
            let elements = doc.querySelectorAll('table.board_list_table tr.table_body');
            debugger;
            for (var i in elements) {
              if (i == 'length') break;
              let item: any = {};
